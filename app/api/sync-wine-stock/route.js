...
export async function GET(req) {
  ...
  try {
        ...
        for (const line of detailData.order.order_lines) {
          ...
          if (extId) {
            ...
            if (!wineQuery.empty) {
              ...
              await setDoc(doc(db, 'wines_sensitive', firebaseId), { stockCount: increment(-deductionAmount) }, { merge: true });
              await setDoc(doc(db, 'wines', firebaseId), { updatedAt: new Date().toISOString() }, { merge: true });
    ...
    // 4. NYT: Gem en log over kørslen i Firebase
    await addDoc(collection(db, 'sync_logs'), {
      createdAt: new Date().toISOString(),
      status: 'success',
      processedCount: updatedWines.length,
      details: updatedWines, // Gemmer hele listen med hvad der blev trukket
    });
    ...
    // Gem også en log, hvis det fejler!
    try {
        await addDoc(collection(db, 'sync_logs'), {
          createdAt: new Date().toISOString(),
          status: 'error',
